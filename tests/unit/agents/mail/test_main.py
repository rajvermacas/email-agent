"""
Unit tests for Mail Agent standalone runner.
"""
...
from info_agent.agents.mail.__main__ import main, run_mail_agent


class TestMainFunction:
    """Tests for main() function and argument parsing."""

    @patch("info_agent.agents.mail.__main__.run_mail_agent")
    def test_main_default_arguments(self, mock_run: MagicMock) -> None:
        """Test main() uses default arguments when none provided."""
        with patch.object(sys, "argv", ["mail"]):
            main()

        mock_run.assert_called_once_with(host="0.0.0.0", port=8002)

    @patch("info_agent.agents.mail.__main__.run_mail_agent")
    def test_main_custom_host(self, mock_run: MagicMock) -> None:
        """Test main() with custom host argument."""
        with patch.object(sys, "argv", ["mail", "--host", "127.0.0.1"]):
            main()

        mock_run.assert_called_once_with(host="127.0.0.1", port=8002)

    @patch("info_agent.agents.mail.__main__.run_mail_agent")
    def test_main_custom_port(self, mock_run: MagicMock) -> None:
        """Test main() with custom port argument."""
        with patch.object(sys, "argv", ["mail", "--port", "9000"]):
            main()
    ...
    def test_main_custom_host_and_port(self, mock_run: MagicMock) -> None:
        ...
        with patch.object(
            sys, "argv", ["mail", "--host", "192.168.1.1", "--port", "8080"]
        ):
            main()
...
class TestRunMailAgent:
    """Tests for run_mail_agent() function."""

    @patch("info_agent.agents.mail.__main__.AgentServer")
    @patch("info_agent.agents.mail.__main__.MailAgentExecutor")
    @patch("info_agent.agents.mail.__main__.EmailStorage")
    def test_creates_email_storage(
        self,
        mock_storage_class: MagicMock,
        mock_executor_class: MagicMock,
        mock_server_class: MagicMock,
    ) -> None:
        """Test that EmailStorage is created."""
        ...
        run_mail_agent(host="localhost", port=8002)
    ...
    def test_creates_executor_with_correct_params(
        ...
        """Test that MailAgentExecutor is created with correct parameters."""
        mock_storage = MagicMock()
        mock_storage_class.return_value = mock_storage
    ...
    def test_creates_server_with_correct_params(
        ...
        run_mail_agent(host="0.0.0.0", port=9000)
    ...
    @patch("info_agent.agents.mail.__main__.signal.signal")
    @patch("info_agent.agents.mail.__main__.AgentServer")
    @patch("info_agent.agents.mail.__main__.MailAgentExecutor")
    @patch("info_agent.agents.mail.__main__.EmailStorage")
...
class TestEndpointConstruction:
    ...
    def test_endpoint_with_localhost(
        ...
        assert call_args[1]["endpoint"] == "http://localhost:8002"
    ...
    def test_endpoint_with_ip_address(
        ...
        run_mail_agent(host="192.168.1.100", port=9999)
